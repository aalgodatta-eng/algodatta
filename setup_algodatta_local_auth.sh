#!/usr/bin/env bash
# =============================================================
#  AlgoDatta Full Local Auth Setup Script (v2.0)
#  ✅ Creates Cognito UserPool, AppClient, Hosted Domain, Users
#  ✅ Generates backend + frontend .env
#  ✅ Patches FastAPI auth routes + dependency
...
# --- Ensure AWS CLI & dependencies ------------------------------------------
if ! command -v aws &>/dev/null; then
  echo "⚙️ Installing AWS CLI..."
  sudo apt update -y && sudo apt install -y awscli
fi

sudo apt install -y jq unzip curl python3-pip

# --- 1️⃣ Create Cognito Pool if not exists ----------------------------------
...
# --- 6️⃣ Patch FastAPI auth modules -----------------------------------------
...
# --- 7️⃣  Install backend deps ----------------------------------------------
pip install fastapi uvicorn requests "python-jose[cryptography]" -q

echo "✅ FastAPI backend patched and dependencies installed"

# --- 8️⃣  Display Summary ---------------------------------------------------
echo ""
echo "============================================================="
echo "🎯 AlgoDatta Local Auth Summary"
echo "-------------------------------------------------------------"
...
echo "  3️⃣  Open the Login URL above to test Cognito login → dashboard redirect."
